...
function findAncestorFolderIds(
  ...
  for (const item of items) {
    ...
    const ancestorIds = findAncestorFolderIds(item.children, targetId, [...path, item.id]);
...
function flattenVisibleTree(
  ...
  return items.flatMap((item) => {
    ...
    return [node, ...flattenVisibleTree(item.children, expandedFolderIds, level + 1, item.id)];
  });
}
      ...
      {items.map((item) => {
        ...
        const rowContent = (
          <>
            ...
            <span
              className={cn("min-w-0 flex-1 truncate", isSelected && "font-medium text-foreground")}
              title={item.name}
            >
              {item.name}
            </span>
...
export function PublicFolderBrowser({
  ...
  const visibleNodeIndex = useMemo(() => {
    return new Map(visibleNodes.map((node, index) => [node.item.id, { node, index }] as const));
  }, [visibleNodes]);
    ...
    <div className="min-w-0 space-y-3 overflow-x-hidden">
      <div className="space-y-1 px-2">
        ...
        <p className="sr-only" id="shared-workspace-tree-help">
          Use the arrow keys to move through files and folders. Press right arrow to expand a
          folder, left arrow to collapse it, and Enter or Space to toggle the focused folder.
        </p>
      </div>
